...
FUNCTION HILBERT,X,D, ANALYTIC = a   ; performs the Hilbert transform of some data.
        ...
        N2=ceil(N/2.)-1             ; effect of odd and even # of elements
                             ; considered here.
        y(0) = complex(0,0)  ; zero the DC value (required for hilbert trans.)
...
FUNCTION gaussian_window, l, w
 if w ne 0.0 then sigma = l/(2*!dPI*w) else MESSAGE,'width is zero!'
 g = dblarr(l)
 iarr = dindgen(l)
 ex = (iarr - l/2)^2/(2*sigma[0]^2)
; probably not needed, I ran into a problem about 6 years ago, and I do not remeber what it was!!
 wl = where(ex lt 25)  
 g(wl) = exp(-ex(wl))
 g = shift(g,-l/2)
 return, dcomplex(g,0)
...
print
PRINT,'Running main level example code'
print,' To see another example type s_trans(/example)'
...
a = s_trans(ts,factor,/verb,/no_ana,/neg,/SAMPLINGRATE)

;!P.multi=[0,1,3]
;plot,ts
;oplot,imaginary(ts),linestyle=1
;contour,abs(a.st),/fill,nlevels=14
;!P.multi=[2,1,6]
;plot,arg(a.st(*,12)),tit='phase(t) for pos freq peak'
;plot,arg(a.st(*,74-12)) ,tit='phase(t) for neg freq peak'
end
